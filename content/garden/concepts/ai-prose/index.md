...
I've been in customer facting technical roles for over a decade now, and for the last 5 years I've been a Sales Engineer. That means, every paragraph I send a customer is an ask for trust. As much as LLM's help me wear all the hats I need to wear, it's made me pause to think about when and how I use LLMs to help me with prose.

## "That Tasted Like Purple!"

{{< youtube MlDkxToyrBk >}}

To stretch the original cake metaphor, the next problem outside of credibility of pretending you hand-baked your cake is it’s “taste”. LLM output drifts toward [purple prose](https://en.wikipedia.org/wiki/Purple_prose), and it’s so widespread now it’s [become memefied](https://englishroseberlin.com/gen-ai-buzzword-bingo/). Out of the box, an LLM will give you a bunch of writing where everything "stands as a testament", every topic is "nuanced", every tool is "robust" and "seamless".
...
Also... I've found this stuff tends to leak. Even when I hand-write something from scratch, sometimes LLM-shaped tendencies can bleed in. Maybe because the sources I'm quoting are themselves AI-assisted, or even some of the patterns are now so widespread they've bleed into society and start feel normal.

[Look how many products have the word "Delve" in them these days!](https://arxiv.org/html/2412.11385v1)
...
Most jobs run on a certain amount of trust, but anything customer facing and *especially* pre-sales technical work, your whole interactions with customers runs on trust. You're building the technical credibility that a customer should use your product/tool/app for their challenges. If a customer reads a note from me and immediately gets that prickling in their thumbs that I'm winging it with AI, that can burn a relationship and, in worse cases, scuttle a whole deal. If that paragraph reads like a vendor landing page, I've already lost ground I can't easily get back, even if the technical content underneath is solid.

Even for non-customer facing roles, and for communication within your own organisation, the overall consequences are mostly lessened but the credibility loss is the same. If I send a colleague a long rambling AI output that I've not properly edited and worked on myself, they're likely going to clock it as AI slop. Then really the subtext for them is **"I didn't respect you enough to check this and write it myself"**.
...
There's also a self-trap I've walked into more than once. If I let the LLM do the first pass and then lightly edit, the underlying "shape" of the message is still the model's, and not my own. I've had a few draft posts when I was figuring out how to use GenAI for my blog last year, and cringe a bit reading back on them now.

I'd rather write something rougher and more honest, even if ultimately it takes longer. At least that roughness is me.

Maybe, I even decide not to use AI at all.

## The Rules I Hold To

So, that's all well and good. But what are some good baseline's to determine when AI output for human-audience text is good or not?

For me, these are my personal smell tests:
...
I've often found especially if the thing I'm writing is a personal point of view, even with some of the tooling I'll be discussing, I find the LLM flattens it out too much and I end up rewriting most of it anyway.
...
Basically the way I see AI works best: LLM for structure and review, human for the actual sentences a blog browser, customer or colleague will read.

## Tooling That Keeps Me Honest

The rules above are mental checklists, but I've now started weaving them in to any AI tooling I'm using to actually make them stick. It's nothing super advanced from an engineering perspective, basically just more context and prompting, via just a few static markdown files that Claude Code et al loads before drafting anything I'll put my name on.
...
I built the first pass by running my this blog through an agent, then hand-edited it over a few months as I noticed things the model missed. The idea is an agent will load this before drafting, so I don't have to constantly add to a prompt to say "Use British spellings, write in first person" etc.
...
For my blog, the actual file lives at [`.claude/references/writing-style.md`](https://github.com/petems/petersouter.xyz/blob/master/.claude/references/writing-style.md).
...
Similar to the writing style, this is kept in the `reference/` directory: For my blog, the actual file lives at [`.claude/references/writing-style.md`](https://github.com/petems/petersouter.xyz/blob/master/.claude/references/writing-style.md).
...
- **Zero Em-Dashes And Zero En-Dashes.** This is an example where I'm actually stricter than the Wikipedia guide, which only flags "overuse of em-dashes". My take is to **ban them outright**, even in number ranges (`5-10 minutes`, not `5 to 10 minutes` with an en-dash). Too strong? Maybe, but maybe that's something that you tweak personally to not be as strict on.
...
As I was tweaking this setup, I found this list of personal preferences is important. Without your voice and personal style being encoded as overrides, you can end up over-correcting and flattening your own style.

### An Example Skill That Wires It All Together

Right now, those are just reference files, and we don't want to have to read and copy paste them into the prompt everytime we're writing prose. What we need something to hook them all together for our agent flow. That's where a [skill](https://docs.claude.com/en/docs/claude-code/skills) comes in!

I wrote one we're talking about here specifically for helping scaffold out blog posts, but I've used this same pattern for writing Conflunce pages, creating handover documents and doing customer research for customer meetings.

For the blog, it takes the voice baseline that I distilled from 10 years worth of posting, and since it's been distilled from the blog anyway it's generally pretty much what I want in terms of tone and style.

I used the same voice distillation for my work Confluence Page creation flow, but for that I wanted to tweak the voice to make things more professional.

So for the confluence page skill, I added an additional reference: `professional-voice-adjustments.md`. This dials the public-voice down for a workplace audience, removing some of the more casual flavour that I'd avoid in a work wiki page. Anyone who knows me knows I'm generally more of an informal kinda-guy, but putting "DO THE DARN THING" as a closer on a post in a work wiki is probably a bit *too* informal.
...
By the end, for my blog, I have a skill that:
...
If you could see a full-detailed diff of the first AI draft suggestion versus what you see now, the main thing that stayed will be the overall structure. The rest is a ship of Theseus situation: it's still something that I consider as "me" writing it, even though I had structural guidance from an LLM.

But it catches far more slop than any editor-pass I've done by hand, and it's been significantly more useful than a bare "Help me write a blog about X" prompt.

## Non-AI Based Prose Critique

One of the areas I've been meaning to work on is what kind of prose-linting and checking did we do before LLM's were so ubiquitous?

After all, if I can leverage that work, what better way to avoid AI-isms than not using AI at all?
...
With customers I think it's just too much of a credibility risk to go "Hey here's some Claude output to start, I'll give a proper response later". It just feels too disrespectful to their time, and ultimately giving a thoughtful, human response is literally the bread-and-butter of being a Sales Engineer, not just copy-pasting ChatGPT!

But the nature of an SE's role is we're constantly jumping from call-to-call, traveling, generally very busy. In the middle of all that, sometimes I get a quick Slack ping from a Sales Exec or colleague in the middle of a quick 5 minute leg-stretch break or in the middle of a call sometimes:

> "What do you know about X?".

There's only so many hours in the day, so sometimes I've not had the chance to properly research it, and I want to give a pointer or some food for thought. For that, I'll probably write something like:
...
That framing does two things: my colleague knows the nature of what they're reading, it gives them a rough sketch of the state of play, and when I follow up with the real answer it means both of us have had that follow-up land with more weight, not less. Disclosing the AI help up front costs me nothing, whereas just copypasta-ing some LLM output is a way to chip way at the trust in our work interactions.

## Closer

None of this is a rallying cry to hand-write everything or [smash the spinning-jenny](https://www.youtube.com/watch?v=N2imgiK2Kqk). Clearly I think there's value in LLM's for human-audience text. But I think having the right processes in place and centering the human as part of the flow of any writing is still key.
...
And with that, somehow I've managed to talk about cake and robots and have a Portal reference. So here's a picture of a cake made by a robot.

[cake-by-a-robot.png]

Portal jokes, they're still relevant and funny in 2026 right?
